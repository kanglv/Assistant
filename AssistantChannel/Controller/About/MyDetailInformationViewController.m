...
- (void)initInforArr{
    ...
    [_informationArr addObject:user.username];
    [_informationArr addObject:user.nickname];
    if([user.sex isEqualToString:@"1"]){
        [_informationArr addObject:@"男"];
    }else{
         [_informationArr addObject:@"女"];
    }
    [_informationArr addObject:user.dep_name];
    [_informationArr addObject:user.role_name];
    [_informationArr addObject:@" "];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row ==([_dataArr count]-1)){
        ...
        cell.rightLabel.text = _informationArr[indexPath.row];
    }
    return cell;
